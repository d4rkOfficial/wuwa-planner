...
>
    ...
    >
        <div class="flex min-w-0 flex-1 flex-col">
            <div class="flex items-center justify-between p-4" style="border-bottom: 1px solid {t.border};">
            ...
            <div class="relative p-3">
                <svg class="pointer-events-none absolute left-6 top-1/2 -translate-y-1/2" width="14" height="14"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round" style="color: {t.textSecondary};">
                    <circle cx="11" cy="11" r="8"></circle>
                    <path d="m21 21-4.35-4.35"></path>
                </svg>
                <input
                    type="text" placeholder="搜索角色（拼音/汉字/英文名）..."
                    bind:value={search}
            ...
            >
                {#each [...grouped.entries()] as [element, presets]}
                    <div class="mb-2 mt-3" data-element={element}>
                        <div class="mb-1 px-1 text-[11px] font-bold uppercase tracking-wider" style="color: {t.textSecondary};">
                        ...
                        {#each presets as preset}
                            <button
                                ...
                                onmouseenter={(e) => (e.currentTarget as HTMLElement).style.background = t.contextHover}
                            ...
                            >
                                <div class="relative h-10 w-10 shrink-0 overflow-hidden rounded-full">
                                    ...
                                    />
                                    <div class="flex h-full w-full items-center justify-center text-sm font-bold" style="color: {t.avatarText};">
                                        {preset.name.charAt(0)}
                                    </div>
                                </div>
                                <div class="flex flex-col">
                                    <div class="flex items-center gap-2">
                                        <span class="text-sm font-bold" style="color: {t.text};">{preset.name}</span>
                                        <StarRating {preset} size={11} />
                                    </div>
                                    <span class="text-xs font-medium flex" style="color: {t.textSecondary};">{preset.nameEn}</span>
                                </div>
                            </button>
                        {/each}
                    </div>
                {:else}
                    <div class="flex h-32 items-center justify-center text-sm" style="color: {t.mutedText};">
                        未找到匹配角色
                    </div>
                {/each}
            </div>
        ...
        <div class="flex flex-col items-center justify-center gap-2 px-2 py-3" style="border-left: 1px solid {t.border};">
            {#each elementOrder as el}
                ...
                >
                    <img src="/images/elements/{el}.png" alt={ELEMENT_TYPE_MAP[el] || el} class="h-full w-full object-cover" />
                </button>
            {/each}
        </div>
    </div>
